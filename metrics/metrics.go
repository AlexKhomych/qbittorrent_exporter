	...
	for i := range val.NumField() {
		...
		if field.Kind() == reflect.Ptr && !field.IsNil() {
			if metric, ok := field.Interface().(prometheus.Collector); ok {
				...
				log.Info("Metrics was registered", "name", fieldType.Name)
			}
		}
	}
}
